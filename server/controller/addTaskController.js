        ...
        var taskId = []
            ...
            data.forEach(doc => {
                const task = new Task(
                    ...
                    doc.data().createdAt,
                    doc.data().createdBy,
                    doc.data().meetingLink,
                    doc.data().isEvent, 
                    doc.data().pickedUptime,
                    doc.data().eventId,
                );
                if(task.userId === req.body.userId) {
                    taskArray.push(task)
                    if(task.isEvent) {
                        taskId.push(task.eventId)
                    }
                }
            });
            try {
                var allEvents = await getEvents(req.body.dateTimeStart, req.body.dateTimeEnd)
            for(let allEventItem of allEvents) {                
                if(!taskId.includes(allEventItem.id)) {
                    console.log('Event add')
                    var params = {
                        taskName: allEventItem.summary,
                        taskSubject: allEventItem.organizer.displayName,
                        userId: req.body.userId,
                        status: 1,                                   
                        createdAt: new Date(),
                        createdBy: allEventItem.creator.email,
                        meetingLink: allEventItem.hangoutLink ? allEventItem.hangoutLink : null,
                        isEvent: true,
                        pickedUptime: allEventItem.start.dateTime,
                        eventId: allEventItem.id,
                    }
                    console.log(params.taskName)
                    await addGoogleTask(params)
...
const cronPickTask = async(req) => {
    try {
        console.log(req)
        const pickedId = req.pickedId
        const pickingId = req.pickingId
        const completedAt = req.completedAt
        const pickedAt = req.pickedAt
        var id = pickedId
        if(id || pickedId === pickingId) {
            const updateTask = await firestore.collection('task').doc(id);
            await updateTask.update({ status: 3, completedAt: completedAt })
        ...
        return { status: 200, message: 'Task Picked'}
    } catch(error) {
        return { status: 400, message: error.message }
    }
}

const cronPickingEvent = async(req, res) => {
    console.log('Cron picking event')
    const task = await firestore.collection('task')
        const data = await task.get();
        var taskProcessId = null
        const taskArray = [];        
            ...
            for(let taskArrayItem of taskArray) {
                if(taskArrayItem.status === 2) {
                    taskProcessId = taskArrayItem.id
                }
                console.log(taskProcessId)                
                if(taskArrayItem.isEvent && new Date(taskArrayItem.pickedUptime) <= new Date()) {
                    var req = {}                    
                    req.pickedId = taskProcessId
                    req.pickingId = taskArrayItem.id
                    req.completedAt = new Date().toString()
                    req.pickedAt = new Date().toString()
                    console.log(req)
                    await cronPickTask(req)
                }
            }
}
